...
Bonitas — API‑инструменты для домофонов. В составе: библиотека, CLI для получения токенов и демо‑веб‑панель. Веб‑демо — страница `/stream` с просмотром камеры и кнопкой открытия двери по 4‑значному коду. Поток идет через RTSP → HLS при наличии `ffmpeg`, иначе показывается превью.

> Бонитет — количественный показатель качества природных объектов, определяющий их экономическую ценность. Термин происходит от лат. *bonitas* («добротность», «высокое качество»).
